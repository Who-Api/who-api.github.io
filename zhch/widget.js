(function() {
    ...
    container.style = "font-family: 'Khmer Sangam MN', -apple-system, sans-serif; max-width: 500px; padding: 25px; border: 1px solid rgba(0,0,0,0.05); border-radius: 12px; background: #ffffff; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); margin: 0 auto;";
    ...
    container.innerHTML = `
        ...
        <div style="display: flex; gap: 8px; margin-bottom: 15px;">
            <input type="text" id="whoisInput" placeholder="输入域名 (例如: apple.com)" style="flex: 1; padding: 12px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 15px; outline: none;" />
            <button id="whoisBtn" style="padding: 12px 24px; background: #000000; color: #ffffff; border: none; border-radius: 6px; font-size: 15px; font-weight: 600; cursor: pointer; white-space: nowrap; transition: background 0.2s;">查询</button>
        </div>
        <div id="whoisLoader" style="display: none; color: #000000; font-style: italic; font-size: 14px; margin-bottom: 10px; font-family: 'Khmer Sangam MN', sans-serif;">正在打开专用套接字...</div>
        <div id="whoisError" style="display: none; color: #ef4444; background: #fef2f2; padding: 12px; border-radius: 6px; font-size: 14px; margin-bottom: 10px; border: 1px solid #fee2e2; line-height: 1.5;"></div>
        <pre id="whoisResult" style="display: none; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0; white-space: pre-wrap; font-size: 12px; color: #1e293b; max-height: 400px; overflow-y: auto; line-height: 1.6;"></pre>
    `;
    ...
    function translateWhoisText(rawText) {
        ...
        // Replace English field names with Chinese, handling case-insensitive and whitespace variations
        for (const [english, chinese] of Object.entries(chineseTranslations)) {
            // Match field names at the start of a line, with optional whitespace, and case-insensitive
            const regex = new RegExp(`^(\\s*)${english.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(\\s*):`, 'gmi');
            translated = translated.replace(regex, `$1${chinese}$2:`);
        }
